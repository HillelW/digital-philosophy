"""
   ...
   see end of Aumann and Maschler's 1985 paper: 
...
class CooperativeGame(object):
    ...
    def empty_set_is_mapped_to_zero(self, valuation) -> bool:
        return valuation[()] == 0
    ...
    # def get_nucleolus(self) -> tuple[float, float, float]:
        ## if core is empty
        # v = self.valuation
        # grand_valuation = v[(1,2,3)]
        # y1 = (grand_valuation + v[(1, 2)] + v[(1,3)] - 2*v[(2, 3)]) / 3
        # y2 = (grand_valuation + v[(1, 2)] + v[(2, 3)] - 2*v[(1, 3)]) / 3
        # y3 = (grand_valuation + v[(2, 3)] + v[(1, 3)] - 2*v[(1, 2)]) / 3
        # return y1, y2, y3

    def get_nucleolus(self) -> tuple[float, float, float]:
        grand_valuation = v[(1,2,3)]
        tups = [(1,2), (1, 3), (2, 3)]
        condition_1 = [grand_valuation >= 3*v[tup] for tup in tups]
        if all(condition_1):
            result = (grand_valuation) / 3
            return result, result, result 
        if (grand_valuation >= v[(1, 2)] + 2*v[(1, 3)] and grand_valuation >= v[(1, 2)] + 
        2*v[(2,3)] and grand_valuation < 3*v[(1, 2)] and
        grand_valuation >= v[(1, 3)] + 2*v[(1, 2)] and grand_valuation >= v[(1, 3)] + 
        2*v[(2,3)] and grand_valuation < 3*v[(1, 3)] and
        grand_valuation >= v[(2, 3)] + 2*v[(1, 2)] and grand_valuation >= v[(2, 3)] + 
        2*v[(1,2)] and grand_valuation < 3*v[(2, 3)]):
            y1 = 
           



    def allocation_problem_to_game(self, estate: float, claims: list[float]):
        """
